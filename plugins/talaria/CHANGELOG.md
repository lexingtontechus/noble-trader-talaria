...
**Polling optimization + PostHog DAU/MAU analytics integration.** REST data fallback poll interval increased from 60s to 300s (5min), reducing per-user daily REST calls by 80% (~1440 → ~288). PostHog analytics SDK integrated via CDN for DAU/MAU tracking, with a single `dashboard_open` event captured on mount.

### Changed

- **Polling interval**: `DATA_POLL_MS` 60s → 300s (5min REST data fallback poll). WebSocket remains the primary signal delivery path; REST polling only fires when the WS connection drops. Status text updated from "60s poll" to "300s poll".
- **Version**: 0.2.11 → 0.2.13 (skipped 0.2.12 — changes were committed but version not bumped)

### Added

- **PostHog analytics**: CDN-lazy-loaded PostHog SDK (`posthog-js` via `us-assets.posthog.com`) for DAU/MAU tracking. Initialized on dashboard mount with `claim.plan_slug` as the distinct ID. Captures `dashboard_open` event with `session_id`, `plan`, and `sub_status` properties.
  - api_host corrected from `us.i.posthog.com` (404) → `us.posthog.com`
  - Project token: `phc_v5U5tCF7ddSmDTtjbZDp3hoV236UjpnKdGNqMWNkjskx`
  - Bundle impact: ~0KB initial (lazy-loaded async via `document.createElement('script')`)
  - No PostHog integration in `talaria-check` edge function (per user: "exclude - talaria-check primary function is to validate tokens")
- **PostHog config constants**: `POSTHOG_TOKEN` and `POSTHOG_API_HOST` added as module-level constants (L39-41)
- **PostHog init useEffect**: Added after claim-check `useEffect`, fires once when `claim.plan_slug` is available (L2845)

### Egress Impact

| Metric | Before (60s) | After (300s) | Reduction |
|---|---|---|---|
| REST calls/user/day | ~1440 | ~288 | -80% |
| Egress/user/day | ~26 MB | ~5.2 MB | -80% |
| 500 users/day | ~13 GB | ~2.6 GB | -80% |

### Verification

- `node --check` (CommonJS): PASS
- `node --check` (ESM via .mjs): PASS
- Render harness: 31/31 PASS — dashboard + connect trees render clean
- PostHog snippet: loads from `https://us-assets.posthog.com/static/array.js` (verified accessible)
