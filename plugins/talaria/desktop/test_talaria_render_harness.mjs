...
globalThis.fetch = async (url) => {
  ...
  // Mocked "latest" bumped to v0.2.19 — deployed PLUGIN_VERSION is now
  // 0.2.17, so the mock must stay strictly ahead for the upgrade-banner scenario below.
  if (u.includes('github.com/repos/lexingtontechus/noble-trader-talaria/releases/latest')) {
    return jsonResp({
      ...
      html_url: 'https://github.com/lexingtontechus/noble-trader-talaria/releases/tag/v0.2.19',
      prerelease: false,
      assets: [
        { name: 'talaria-plugin-v0.2.19.zip', browser_download_url: 'https://github.com/lexingtontechus/noble-trader-talaria/releases/download/v0.2.19/talaria-plugin-v0.2.19.zip' },
      ],
    })
  }
...
assert(paneFootAcc.texts.some((x) => x.includes('Talaria v0.2.18')), 'pane footer shows plugin version v0.2.18')
...
// latest release. The mock returns v0.2.19 > deployed PLUGIN_VERSION 0.2.18,
...
assert(upgradeAcc.texts.some((x) => x.includes('Upgrade available · v0.2.19')), 'upgrade banner shows latest release tag (v0.2.19)')
